...
This is the new structure of the patient data
...
----------------------
* This table contains the ```admission_id```, the ```PrimaryDiagnosisCode```, and the ```PrimaryDiagnosisDescription```
...
This is the new structure of the diagnosis data
...
----------------------
* This table currently contains ```admission_id```, ```LabName```, ```LabValue```, ```LabUnits```, and ```LabDateTime``` fields
...
* We can create a dimension table called ```lab_type_dim``` with one row for each lab type (35 total)
   ...
   * The table will also include ```units``` and ```lab_type_name``` from the original ```lab``` table
   ...
   Storage estimates for the new data
   Table | Column | Est. bytes used | # Rows | Total storage used
   ------|--------|-----------------|---------|-------------------
